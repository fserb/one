 ...
 *
 * `deno bundle` has no metafile, so this reads the source map: --sourcemap
 * names, for every run of output bytes, the input file it was minified from.
 * Summing those runs per source is exact, and the columns add up to the file.
 *
...
// Base64 VLQ: six bits at a time, the top one saying another group follows and
// the lowest bit of the first group the sign.
function* vlq(field) {
  let n = 0;
  let shift = 0;
