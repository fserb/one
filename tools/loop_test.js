 ...
 *
 * The cut is the one part of the recorder whose correctness is not obvious
 * from watching. These run on made-up signatures: no canvas, no browser.
 *
...
// `period` frames of cyclic motion, plus a monotonic ramp over `cells` of them
// totalling `ramp` luminance, standing in for a score that only counts up.
...
// findLoop's contract: the seam costs no more than TOL ordinary steps.
...
// A real loop exists, so it must not collapse to the minimum, and the seam it
// picks must hold the contract.
...
// The same with a score climbing 80 luminance in two cells: the case absolute
// minimisation collapsed to the shortest clip.
for (const p of [60, 90]) {
...
// Everything ramps and nothing repeats: no seam holds, so return the least bad
// cut rather than pretend one loops.
{
  const c = findLoop(take(300, 1e9, 400, N), LO, HI);
  check("no loop exists -> shortest", len(c) === LO, `len ${len(c)}`);
}

// A frozen take has no gameplay at any length. What matters is not which cut
// comes back but that it has no motion, so `keep` refuses it.
{
...
// A round that ends early: live frames, then identical ones. The frozen span
// costs nothing to cut across and is the longest thing going, so the search
// used to land inside it and hand back a still.
{
  const live = (i) => {
    const v = new Float32Array(N);
    for (let c = 0; c < N; c++) {
  ...
  // The take as a whole moves, so measuring it and not the cut is what let a
  // frozen cut through.
...
// A take with no input is a still, whatever findLoop makes of it.
